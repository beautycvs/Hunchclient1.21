...
// DarkMode fullscreen overlay shader
// Applies tint, saturation, contrast, vignette, and chromatic aberration
...
void main() {
    ...
    if (ChromaticAberration > 0.001) {
        ...
        worldColor.r = texture(Sampler0, uv + offset * 0.5).r;
        worldColor.g = texture(Sampler0, uv).g;
        worldColor.b = texture(Sampler0, uv - offset * 0.5).b;
    } else {
        worldColor = texture(Sampler0, uv).rgb;
    }
