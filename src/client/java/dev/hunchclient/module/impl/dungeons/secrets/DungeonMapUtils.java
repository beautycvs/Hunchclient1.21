...
/** Dungeon map utilities for converting between map space and world space. */
public class DungeonMapUtils {
	...
	@Nullable
	private static Vector2i getMapPlayerPos(MapItemSavedData map) {
	...
	@Nullable
	public static ObjectIntPair<Vector2ic> getMapEntrancePosAndRoomSize(@NotNull MapItemSavedData map) {
	 ...
	 */
	@Nullable
	public static Vector2ic getMapRoomPos(MapItemSavedData map, Vector2ic mapEntrancePos, int mapRoomSize) {
	 ...
	 */
	@NotNull
	public static Vector2ic getPhysicalRoomPos(@NotNull Vec3 pos) {
	...
	public static Room.Type getRoomType(MapItemSavedData map, Vector2ic mapPos) {
	...
	public static Vector2ic[] getRoomSegments(MapItemSavedData map, Vector2ic mapPos, int mapRoomSize, byte color) {
		...
		HunchClient.LOGGER.debug("[DungeonMapUtils] Found dungeon room segments: {}", Arrays.toString(segments.toArray()));
